...
export default async function AdminBookingDetailPage({
  ...
  return (
    <div className="max-w-5xl">
      <Link href="/admin/bookings" className="inline-flex items-center gap-2 text-xs font-mono uppercase tracking-widest text-slate-400 hover:text-ocean mb-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
        <Card label="Guest">
          {booking.user_id ? (
            <Link href={`/admin/users/${booking.user_id}`} className="block hover:text-ocean">
              ...
              <div className="text-xs text-slate-400 font-mono">{booking.user_email}</div>
            </Link>
          ) : (
        ...
        <Card label="Guests">
          ...
          <div className="text-xs text-slate-400 mt-0.5">max {booking.property_max_guests} on this property</div>
        </Card>
        <Card label="Pricing">
          ...
          <div className="text-xs text-slate-400 mt-0.5">paid: {eur(booking.paid_cents)} · outstanding: {eur(Math.max(0, booking.agreed_total_cents - booking.paid_cents))}</div>
        </Card>
      </div>

      {booking.status === 'cancelled' && booking.cancelled_at && (
        <section className="mb-10 rounded-2xl bg-rose-50 border border-rose-200 p-5">
          <h2 className="text-xs font-mono uppercase tracking-widest text-rose-800 mb-2">Cancellation</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            <div>
              <div className="font-mono text-xs uppercase tracking-widest text-rose-700/60">By</div>
              <div className="text-slate-900 font-bold">{booking.cancelled_by}</div>
            </div>
            <div>
              <div className="font-mono text-xs uppercase tracking-widest text-rose-700/60">Refund owed</div>
              <div className="text-slate-900 font-bold">
                {eur(booking.refund_amount_cents ?? 0)}{' '}
                <span className="text-rose-700/60 text-xs">({booking.policy_applied})</span>
              </div>
              <div className="text-xs text-rose-700/60">refunded: {eur(booking.refunded_cents)}</div>
            </div>
            <div>
              <div className="font-mono text-xs uppercase tracking-widest text-rose-700/60">When</div>
              <div className="text-slate-900 font-bold">{fmtDateTime(booking.cancelled_at)}</div>
            </div>
          </div>
          {booking.cancellation_reason && (
            <div className="mt-3 text-xs text-rose-900 italic">&ldquo;{booking.cancellation_reason}&rdquo;</div>
          )}
        </section>
      )}

      <section className="mb-10">
        <h2 className="text-xs font-mono uppercase tracking-widest text-slate-400 mb-3">Record payment (cash · admin)</h2>
      ...
      <section className="mb-10">
        <h2 className="text-xs font-mono uppercase tracking-widest text-slate-400 mb-3">Payments ({payments.length})</h2>
                    ...
                    <tr key={p.id} className="border-t border-slate-50">
                      ...
                      <td className="px-4 py-2">
                        {p.method === 'stripe' && stripeUrl ? (
                          <a href={stripeUrl} target="_blank" rel="noreferrer noopener" className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded font-mono text-xs bg-violet-50 text-violet-800 ring-1 ring-violet-200">
                            stripe <ExternalLink className="w-2.5 h-2.5" />
                          </a>
                        ) : (
                          <span className="inline-flex items-center px-1.5 py-0.5 rounded font-mono text-xs bg-amber-50 text-amber-800 ring-1 ring-amber-200">cash</span>
                        )}
                      </td>
                      <td className="px-4 py-2">
                        <span className={`inline-flex items-center px-1.5 py-0.5 rounded font-mono text-xs ring-1 ${
                      ...
                      <td className="px-4 py-2 text-right text-xs text-slate-400">{fmtDateTime(p.paid_at)}</td>
                      <td className="px-4 py-2 text-right">
                        {p.method === 'cash' && p.status === 'pending' && (
                          <form action={markCashReceived} className="inline-flex">
                            <input type="hidden" name="payment_id" value={p.id} />
                            <button type="submit" className="px-2.5 py-1 text-xs font-mono uppercase tracking-widest rounded bg-emerald-600 text-white hover:bg-emerald-700 transition">
                              Mark received
                            </button>
                          </form>
                        )}
                        {p.method === 'stripe' && p.status === 'succeeded' && p.amount_cents > p.refunded_cents && (
                          <form action={refundStripePayment} className="inline-flex">
                            <input type="hidden" name="payment_id" value={p.id} />
                            <button type="submit" className="px-2.5 py-1 text-xs font-mono uppercase tracking-widest rounded bg-white border border-rose-200 text-rose-700 hover:bg-rose-50 transition">
                              Refund full
                            </button>
                          </form>
                        )}
      ...
      <section>
        <h2 className="text-xs font-mono uppercase tracking-widest text-slate-400 mb-3">Audit timeline ({events.length})</h2>
        <ol className="space-y-2">
          {events.map((e) => (
            <li key={e.id} className="rounded-xl bg-white border border-slate-100 px-5 py-3 flex items-start justify-between gap-3">
              <div>
                <div className="text-xs font-mono uppercase tracking-widest text-ocean">{e.event_type}</div>
                {Object.keys(e.payload).length > 0 && (
                  <pre className="text-xs font-mono text-slate-400 mt-1 whitespace-pre-wrap">{JSON.stringify(e.payload, null, 2)}</pre>
                )}
              </div>
              <span className="text-xs text-slate-400 shrink-0">{fmtDateTime(e.created_at)}</span>
            </li>
          ))}
        </ol>
      </section>
...
function Card({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="rounded-2xl bg-white border border-slate-100 p-4">
      <h3 className="text-xs font-mono uppercase tracking-widest text-slate-400 mb-2">{label}</h3>
      {children}
    </div>
  );
}
